public class SecurityPassMaker
{
    public string GetDisplayName(TeamSupport support)
    {
        if(support is Staff)
        {
            if(support is Security && CheckIsPriorityMember(support))
            {
               return $"{((Security)support).Title} Priority Personnel";
            }
    ...
    private bool CheckIsPriorityMember(TeamSupport member)
        => (member.GetType() != typeof(SecurityJunior) 
                   && member.GetType() != typeof(SecurityIntern) 
                   && member.GetType() != typeof(PoliceLiaison));
